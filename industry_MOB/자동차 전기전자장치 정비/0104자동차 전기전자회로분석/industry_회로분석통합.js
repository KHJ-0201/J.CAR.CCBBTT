...
async function loadQuestions() {
  try {
    const module = await import('./industry_회로분석문제.js');
    ...
    if (round === '0') {
        const allSets = [
            ...(module.industry010401 || []),
            ...(module.industry010402 || []),
            ...(module.industry010403 || []),
            ...(module.industry010404 || []),
            ...(module.industry010405 || []),
      ];
    selectedSet = shuffleArray(allSets).slice(0, 60);
    roundTitleText = '전기전자회로분석 전체 회차 랜덤 60문제';
    }   else if (!isNaN(round)) {
            selectedSet = module[`industry01040${round}`];
            roundTitleText = `전기전자회로분석 ${round}회차`;
    }   else {
            return;
    }
...
function submitQuiz() {
    ...
    status.innerHTML = `<span id="scoreDisplay">전기전자회로분석 ${round}회차 총점: ${score}/${questions.length}</span>`;

    document.getElementById("submitBtn").style.display = "none";

    const resultDiv = document.getElementById("result");
    resultDiv.innerHTML = `<h2>전기전자회로분석 ${round}회차 총점: ${score}/${questions.length}</h2>`;
}
