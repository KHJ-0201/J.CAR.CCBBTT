...
async function loadQuestions() {
  try {
    const module = await import('./industry_네트워크문제.js');
    ...
    if (round === '0') {
        const allSets = [
            ...(module.industry010801 || []),
            ...(module.industry010802 || []),
            ...(module.industry010803 || []),
            ...(module.industry010804 || []),
            ...(module.industry010805 || []),
      ];
    selectedSet = shuffleArray(allSets).slice(0, 60);
    roundTitleText = '네트워크장비정비 전체 회차 랜덤 60문제';
    }   else if (!isNaN(round)) {
            selectedSet = module[`industry01080${round}`];
            roundTitleText = `네트워크장비정비 ${round}회차`;
    }   else {
            return;
    }
...
function submitQuiz() {
    ...
    status.innerHTML = `<span id="scoreDisplay">네트워크장비정비 ${round}회차 총점: ${score}/${questions.length}</span>`;

    document.getElementById("submitBtn").style.display = "none";

    const resultDiv = document.getElementById("result");
    resultDiv.innerHTML = `<h2>네트워크장비정비 ${round}회차 총점: ${score}/${questions.length}</h2>`;
}
