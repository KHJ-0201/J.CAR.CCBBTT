...
async function loadQuestions() {
  try {
    const module = await import('./industry_편의장치문제.js');
    ...
    if (round === '0') {
        const allSets = [
            ...(module.industry010501 || []),
            ...(module.industry010502 || []),
            ...(module.industry010503 || []),
            ...(module.industry010504 || []),
            ...(module.industry010505 || []),
      ];
    selectedSet = shuffleArray(allSets).slice(0, 60);
    roundTitleText = '편의장치정비 전체 회차 랜덤 60문제';
    }   else if (!isNaN(round)) {
            selectedSet = module[`industry01050${round}`];
            roundTitleText = `편의장치정비 ${round}회차`;
    }   else {
            return;
    }
...
function submitQuiz() {
    ...
    status.innerHTML = `<span id="scoreDisplay">편의장치정비 ${round}회차 총점: ${score}/${questions.length}</span>`;

    document.getElementById("submitBtn").style.display = "none";

    const resultDiv = document.getElementById("result");
    resultDiv.innerHTML = `<h2>편의장치정비 ${round}회차 총점: ${score}/${questions.length}</h2>`;
}
