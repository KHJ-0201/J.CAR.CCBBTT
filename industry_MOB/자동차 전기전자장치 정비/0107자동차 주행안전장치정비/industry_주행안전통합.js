...
async function loadQuestions() {
  try {
    ...
    if (round === '0') {
        const allSets = [
            ...(module.industry010701 || []),
            ...(module.industry010702 || []),
            ...(module.industry010703 || []),
            ...(module.industry010704 || []),
            ...(module.industry010705 || []),
      ];
    selectedSet = shuffleArray(allSets).slice(0, 60);
    roundTitleText = '주행안전장치정비 전체 회차 랜덤 60문제';
    }   else if (!isNaN(round)) {
            selectedSet = module[`industry01070${round}`];
            roundTitleText = `주행안전장치정비 ${round}회차`;
    }   else {
            return;
    }
...
function submitQuiz() {
    ...
    status.innerHTML = `<span id="scoreDisplay">주행안전장치정비 ${round}회차 총점: ${score}/${questions.length}</span>`;

    document.getElementById("submitBtn").style.display = "none";

    const resultDiv = document.getElementById("result");
    resultDiv.innerHTML = `<h2>주행안전장치정비 ${round}회차 총점: ${score}/${questions.length}</h2>`;
}
