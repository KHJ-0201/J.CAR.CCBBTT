...
async function loadQuestions() {
  try {
    const module = await import('./industry_하이브리드문제.js');
    ...
    if (round === '0') {
        const allSets = [
            ...(module.industry010901 || []),
            ...(module.industry010902 || []),
            ...(module.industry010903 || []),
            ...(module.industry010904 || []),
            ...(module.industry010905 || []),
      ];
    selectedSet = shuffleArray(allSets).slice(0, 60);
    roundTitleText = '하이브리드 자동차 특화시스템 정비 전체 회차 랜덤 60문제';
    }   else if (!isNaN(round)) {
            selectedSet = module[`industry01090${round}`];
            roundTitleText = `하이브리드 자동차 특화시스템 정비 ${round}회차`;
    }   else {
            return;
    }
...
function submitQuiz() {
    ...
    status.innerHTML = `<span id="scoreDisplay">하이브리드자동차특화시스템정비 ${round}회차 총점: ${score}/${questions.length}</span>`;

    document.getElementById("submitBtn").style.display = "none";

    const resultDiv = document.getElementById("result");
    resultDiv.innerHTML = `<h2>하이브리드자동차특화시스템정비 ${round}회차 총점: ${score}/${questions.length}</h2>`;
}
