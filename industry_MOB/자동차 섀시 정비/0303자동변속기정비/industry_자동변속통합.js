...
async function loadQuestions() {
  try {
    const module = await import('./industry_자동변속문제.js');
    ...
    if (round === '0') {
        const allSets = [
            ...(module.industry030301 || []),
            ...(module.industry030302 || []),
            ...(module.industry030303 || []),
            ...(module.industry030304 || []),
            ...(module.industry030305 || []),
      ];
    selectedSet = shuffleArray(allSets).slice(0, 60);
    roundTitleText = '자동변속 전체 회차 랜덤 60문제';
    }   else if (!isNaN(round)) {
            selectedSet = module[`industry03030${round}`];
            roundTitleText = `자동변속 ${round}회차`;
    }   else {
            return;
    }
...
function submitQuiz() {
    ...
    status.innerHTML = `<span id="scoreDisplay">자동변속기정비 ${round}회차 총점: ${score}/${questions.length}</span>`;

    document.getElementById("submitBtn").style.display = "none";

    const resultDiv = document.getElementById("result");
    resultDiv.innerHTML = `<h2>자동변속기정비 ${round}회차 총점: ${score}/${questions.length}</h2>`;
}
