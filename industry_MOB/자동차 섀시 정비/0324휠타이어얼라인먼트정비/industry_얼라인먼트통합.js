...
async function loadQuestions() {
  try {
    const module = await import('./industry_얼라인먼트문제.js');
    ...
    if (round === '0') {
        const allSets = [
            ...(module.industry032401 || []),
            ...(module.industry032402 || []),
            ...(module.industry032403 || []),
            ...(module.industry032404 || []),
            ...(module.industry032405 || []),
      ];
    selectedSet = shuffleArray(allSets).slice(0, 60);
    roundTitleText = '얼라인먼트 전체 회차 랜덤 60문제';
    }   else if (!isNaN(round)) {
            selectedSet = module[`industry03240${round}`];
            roundTitleText = `얼라인먼트 ${round}회차`;
    }   else {
            return;
    }
...
function submitQuiz() {
    ...
    status.innerHTML = `<span id="scoreDisplay">휠·타이어·얼라인먼트 정비 ${round}회차 총점: ${score}/${questions.length}</span>`;

    document.getElementById("submitBtn").style.display = "none";

    const resultDiv = document.getElementById("result");
    resultDiv.innerHTML = `<h2>휠·타이어·얼라인먼트 정비 ${round}회차 총점: ${score}/${questions.length}</h2>`;
}
