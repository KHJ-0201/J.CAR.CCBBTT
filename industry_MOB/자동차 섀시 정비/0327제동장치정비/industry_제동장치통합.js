...
async function loadQuestions() {
  try {
    const module = await import('./industry_제동장치문제.js');
    ...
    if (round === '0') {
        const allSets = [
            ...(module.industry032701 || []),
            ...(module.industry032702 || []),
            ...(module.industry032703 || []),
            ...(module.industry032704 || []),
            ...(module.industry032705 || []),
      ];
    selectedSet = shuffleArray(allSets).slice(0, 60);
    roundTitleText = '제동장치 전체 회차 랜덤 60문제';
    }   else if (!isNaN(round)) {
            selectedSet = module[`industry03270${round}`];
            roundTitleText = `제동장치 ${round}회차`;
    }   else {
            return;
    }
...
function submitQuiz() {
    ...
    status.innerHTML = `<span id="scoreDisplay">제동장치 정비 ${round}회차 총점: ${score}/${questions.length}</span>`;

    document.getElementById("submitBtn").style.display = "none";

    const resultDiv = document.getElementById("result");
    resultDiv.innerHTML = `<h2>제동장치 정비 ${round}회차 총점: ${score}/${questions.length}</h2>`;
}
