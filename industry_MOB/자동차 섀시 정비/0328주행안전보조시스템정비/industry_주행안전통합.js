...
async function loadQuestions() {
  try {
    const module = await import('./industry_주행안전문제.js');
    ...
    if (round === '0') {
        const allSets = [
            ...(module.industry032801 || []),
            ...(module.industry032802 || []),
            ...(module.industry032803 || []),
            ...(module.industry032804 || []),
            ...(module.industry032805 || []),
      ];
    selectedSet = shuffleArray(allSets).slice(0, 60);
    roundTitleText = '주행안전 전체 회차 랜덤 60문제';
    }   else if (!isNaN(round)) {
            selectedSet = module[`industry03280${round}`];
            roundTitleText = `주행안전 ${round}회차`;
    }   else {
            return;
    }
...
function submitQuiz() {
    ...
    status.innerHTML = `<span id="scoreDisplay">주행안전보조시스템 정비 ${round}회차 총점: ${score}/${questions.length}</span>`;

    document.getElementById("submitBtn").style.display = "none";

    const resultDiv = document.getElementById("result");
    resultDiv.innerHTML = `<h2>주행안전보조시스템 정비 ${round}회차 총점: ${score}/${questions.length}</h2>`;
}
