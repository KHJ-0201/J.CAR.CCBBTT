...
async function loadQuestions() {
  try {
    const module = await import('./industry_전자제어현가문제.js');
    ...
    if (round === '0') {
        const allSets = [
            ...(module.industry030901 || []),
            ...(module.industry030902 || []),
            ...(module.industry030903 || []),
            ...(module.industry030904 || []),
            ...(module.industry030905 || []),
      ];
    selectedSet = shuffleArray(allSets).slice(0, 60);
    roundTitleText = '전자제어현가 전체 회차 랜덤 60문제';
    }   else if (!isNaN(round)) {
            selectedSet = module[`industry03090${round}`];
            roundTitleText = `전자제어현가 ${round}회차`;
    }   else {
            return;
    }
...
function submitQuiz() {
    ...
    status.innerHTML = `<span id="scoreDisplay">전자제어 현가장치 정비 ${round}회차 총점: ${score}/${questions.length}</span>`;

    document.getElementById("submitBtn").style.display = "none";

    const resultDiv = document.getElementById("result");
    resultDiv.innerHTML = `<h2>전자제어 현가장치 정비 ${round}회차 총점: ${score}/${questions.length}</h2>`;
}
