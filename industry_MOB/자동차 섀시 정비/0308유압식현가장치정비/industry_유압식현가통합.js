...
async function loadQuestions() {
  try {
    const module = await import('./industry_유압식현가문제.js');
    ...
    if (round === '0') {
        const allSets = [
            ...(module.industry030801 || []),
            ...(module.industry030802 || []),
            ...(module.industry030803 || []),
            ...(module.industry030804 || []),
            ...(module.industry030805 || []),
      ];
    selectedSet = shuffleArray(allSets).slice(0, 60);
    roundTitleText = '유압식현가 전체 회차 랜덤 60문제';
    }   else if (!isNaN(round)) {
            selectedSet = module[`industry03080${round}`];
            roundTitleText = `유압식현가 ${round}회차`;
    }   else {
            return;
    }
...
function submitQuiz() {
    ...
    status.innerHTML = `<span id="scoreDisplay">유압식현가장치정비 ${round}회차 총점: ${score}/${questions.length}</span>`;

    document.getElementById("submitBtn").style.display = "none";

    const resultDiv = document.getElementById("result");
    resultDiv.innerHTML = `<h2>유압식현가장치정비 ${round}회차 총점: ${score}/${questions.length}</h2>`;
}
