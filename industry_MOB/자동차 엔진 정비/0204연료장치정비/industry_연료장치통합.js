...
async function loadQuestions() {
  try {
    const module = await import('./industry_연료장치문제.js');
    ...
    if (round === '0') {
        const allSets = [
            ...(module.industry020401 || []),
            ...(module.industry020402 || []),
            ...(module.industry020403 || []),
            ...(module.industry020404 || []),
            ...(module.industry020405 || []),
      ];
    selectedSet = shuffleArray(allSets).slice(0, 60);
    roundTitleText = '연료장치 전체 회차 랜덤 60문제';
    }   else if (!isNaN(round)) {
            selectedSet = module[`industry02040${round}`];
            roundTitleText = `연료장치 ${round}회차`;
    }   else {
            return;
    }
...
function submitQuiz() {
    ...
    status.innerHTML = `<span id="scoreDisplay">연료장치 정비 ${round}회차 총점: ${score}/${questions.length}</span>`;

    document.getElementById("submitBtn").style.display = "none";

    const resultDiv = document.getElementById("result");
    resultDiv.innerHTML = `<h2>연료장치 정비 ${round}회차 총점: ${score}/${questions.length}</h2>`;
}
