...
async function loadQuestions() {
  try {
    const module = await import('./industry_배출가스문제.js');
    ...
    if (round === '0') {
        const allSets = [
            ...(module.industry021201 || []),
            ...(module.industry021202 || []),
            ...(module.industry021203 || []),
            ...(module.industry021204 || []),
            ...(module.industry021205 || []),
      ];
    selectedSet = shuffleArray(allSets).slice(0, 60);
    roundTitleText = '배출가스제어장치 전체 회차 랜덤 60문제';
    }   else if (!isNaN(round)) {
            selectedSet = module[`industry02120${round}`];
            roundTitleText = `배출가스제어장치 ${round}회차`;
    }   else {
            return;
    }
...
function submitQuiz() {
    ...
    status.innerHTML = `<span id="scoreDisplay">배출가스제어장치 정비 ${round}회차 총점: ${score}/${questions.length}</span>`;

    document.getElementById("submitBtn").style.display = "none";

    const resultDiv = document.getElementById("result");
    resultDiv.innerHTML = `<h2>배출가스제어장치 정비 ${round}회차 총점: ${score}/${questions.length}</h2>`;
}
