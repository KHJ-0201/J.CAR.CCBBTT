...
async function loadQuestions() {
  try {
    const module = await import('./industry_과급장치문제.js');
    ...
    if (round === '0') {
        const allSets = [
            ...(module.industry020701 || []),
            ...(module.industry020702 || []),
            ...(module.industry020703 || []),
            ...(module.industry020704 || []),
            ...(module.industry020705 || []),
      ];
    selectedSet = shuffleArray(allSets).slice(0, 60);
    roundTitleText = '과급장치 전체 회차 랜덤 60문제';
    }   else if (!isNaN(round)) {
            selectedSet = module[`industry02070${round}`];
            roundTitleText = `과급장치 ${round}회차`;
    }   else {
            return;
    }
...
function submitQuiz() {
    ...
    status.innerHTML = `<span id="scoreDisplay">과급장치 정비 ${round}회차 총점: ${score}/${questions.length}</span>`;

    document.getElementById("submitBtn").style.display = "none";

    const resultDiv = document.getElementById("result");
    resultDiv.innerHTML = `<h2>과급장치 정비 ${round}회차 총점: ${score}/${questions.length}</h2>`;
}
