...
async function loadQuestions() {
  try {
    const module = await import('./industry_엔진본체문제.js');
    ...
    if (round === '0') {
        const allSets = [
            ...(module.industry020101 || []),
            ...(module.industry020102 || []),
            ...(module.industry020103 || []),
            ...(module.industry020104 || []),
            ...(module.industry020105 || []),
      ];
    selectedSet = shuffleArray(allSets).slice(0, 60);
    roundTitleText = '엔진본체 전체 회차 랜덤 60문제';
    }   else if (!isNaN(round)) {
            selectedSet = module[`industry02010${round}`];
            roundTitleText = `엔진본체 ${round}회차`;
    }   else {
            return;
    }
...
function submitQuiz() {
    ...
    status.innerHTML = `<span id="scoreDisplay">엔진본체 정비 ${round}회차 총점: ${score}/${questions.length}</span>`;

    document.getElementById("submitBtn").style.display = "none";

    const resultDiv = document.getElementById("result");
    resultDiv.innerHTML = `<h2>엔진본체 정비 ${round}회차 총점: ${score}/${questions.length}</h2>`;
}
