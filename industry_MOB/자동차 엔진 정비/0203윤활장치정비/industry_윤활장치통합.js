...
async function loadQuestions() {
  try {
    const module = await import('./industry_윤활장치문제.js');
    ...
    if (round === '0') {
        const allSets = [
            ...(module.industry020301 || []),
            ...(module.industry020302 || []),
            ...(module.industry020303 || []),
            ...(module.industry020304 || []),
            ...(module.industry020305 || []),
      ];
    selectedSet = shuffleArray(allSets).slice(0, 60);
    roundTitleText = '윤활장치 전체 회차 랜덤 60문제';
    }   else if (!isNaN(round)) {
            selectedSet = module[`industry02030${round}`];
            roundTitleText = `윤활장치 ${round}회차`;
    }   else {
            return;
    }
...
function submitQuiz() {
    ...
    status.innerHTML = `<span id="scoreDisplay">윤활장치 정비 ${round}회차 총점: ${score}/${questions.length}</span>`;

    document.getElementById("submitBtn").style.display = "none";

    const resultDiv = document.getElementById("result");
    resultDiv.innerHTML = `<h2>윤활장치 정비 ${round}회차 총점: ${score}/${questions.length}</h2>`;
}
