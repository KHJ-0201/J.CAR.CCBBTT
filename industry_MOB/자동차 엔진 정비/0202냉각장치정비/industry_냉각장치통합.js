...
async function loadQuestions() {
  try {
    const module = await import('./industry_냉각장치문제.js');
    ...
    if (round === '0') {
        const allSets = [
            ...(module.industry020201 || []),
            ...(module.industry020202 || []),
            ...(module.industry020203 || []),
            ...(module.industry020204 || []),
            ...(module.industry020205 || []),
      ];
    selectedSet = shuffleArray(allSets).slice(0, 60);
    roundTitleText = '냉각장치 전체 회차 랜덤 60문제';
    }   else if (!isNaN(round)) {
            selectedSet = module[`industry02020${round}`];
            roundTitleText = `냉각장치 ${round}회차`;
    }   else {
            return;
    }
...
function submitQuiz() {
    ...
    status.innerHTML = `<span id="scoreDisplay">냉각장치 정비 ${round}회차 총점: ${score}/${questions.length}</span>`;

    document.getElementById("submitBtn").style.display = "none";

    const resultDiv = document.getElementById("result");
    resultDiv.innerHTML = `<h2>냉각장치 정비 ${round}회차 총점: ${score}/${questions.length}</h2>`;
}
