...
async function loadQuestions() {
  try {
    const module = await import('./industry_가솔린전자문제.js');
    ...
    if (round === '0') {
        const allSets = [
            ...(module.industry020801 || []),
            ...(module.industry020802 || []),
            ...(module.industry020803 || []),
            ...(module.industry020804 || []),
            ...(module.industry020805 || []),
      ];
    selectedSet = shuffleArray(allSets).slice(0, 60);
    roundTitleText = '가솔린 전자제어 장치 전체 회차 랜덤 60문제';
    }   else if (!isNaN(round)) {
            selectedSet = module[`industry02080${round}`];
            roundTitleText = `가솔린 전자제어 장치 ${round}회차`;
    }   else {
            return;
    }
...
function submitQuiz() {
    ...
    status.innerHTML = `<span id="scoreDisplay">가솔린전자제어장치 정비 ${round}회차 총점: ${score}/${questions.length}</span>`;

    document.getElementById("submitBtn").style.display = "none";

    const resultDiv = document.getElementById("result");
    resultDiv.innerHTML = `<h2>가솔린전자제어장치 정비 ${round}회차 총점: ${score}/${questions.length}</h2>`;
}
