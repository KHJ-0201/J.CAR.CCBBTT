...
async function loadQuestions() {
  try {
    const module = await import('./industry_엔진점화장치문제.js');
    ...
    if (round === '0') {
        const allSets = [
            ...(module.industry020501 || []),
            ...(module.industry020502 || []),
            ...(module.industry020503 || []),
            ...(module.industry020504 || []),
            ...(module.industry020505 || []),
      ];
    selectedSet = shuffleArray(allSets).slice(0, 60);
    roundTitleText = '엔진점화장치 전체 회차 랜덤 60문제';
    }   else if (!isNaN(round)) {
            selectedSet = module[`industry02050${round}`];
            roundTitleText = `엔진점화장치 ${round}회차`;
    }   else {
            return;
    }
...
function submitQuiz() {
    ...
    status.innerHTML = `<span id="scoreDisplay">엔진점화장치 정비 ${round}회차 총점: ${score}/${questions.length}</span>`;

    document.getElementById("submitBtn").style.display = "none";

    const resultDiv = document.getElementById("result");
    resultDiv.innerHTML = `<h2>엔진점화장치 정비 ${round}회차 총점: ${score}/${questions.length}</h2>`;
}
