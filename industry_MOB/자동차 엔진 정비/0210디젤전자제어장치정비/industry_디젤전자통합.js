...
async function loadQuestions() {
  try {
    const module = await import('./industry_디젤전자문제.js');
    ...
    if (round === '0') {
        const allSets = [
            ...(module.industry021001 || []),
            ...(module.industry021002 || []),
            ...(module.industry021003 || []),
            ...(module.industry021004 || []),
            ...(module.industry021005 || []),
      ];
    selectedSet = shuffleArray(allSets).slice(0, 60);
    roundTitleText = '디젤 전자제어 장치 전체 회차 랜덤 60문제';
    }   else if (!isNaN(round)) {
            selectedSet = module[`industry02100${round}`];
            roundTitleText = `디젤 전자제어 장치 ${round}회차`;
    }   else {
            return;
    }
...
function submitQuiz() {
    ...
    status.innerHTML = `<span id="scoreDisplay">디젤전자제어장치 정비 ${round}회차 총점: ${score}/${questions.length}</span>`;

    document.getElementById("submitBtn").style.display = "none";

    const resultDiv = document.getElementById("result");
    resultDiv.innerHTML = `<h2>디젤전자제어장치 정비 ${round}회차 총점: ${score}/${questions.length}</h2>`;
}
