...
async function loadQuestions() {
  try {
    const module = await import('./industry_흡배기제어장치문제.js');
    ...
    if (round === '0') {
        const allSets = [
            ...(module.industry020601 || []),
            ...(module.industry020602 || []),
            ...(module.industry020603 || []),
            ...(module.industry020604 || []),
            ...(module.industry020605 || []),
      ];
    selectedSet = shuffleArray(allSets).slice(0, 60);
    roundTitleText = '흡배기 제어장치 전체 회차 랜덤 60문제';
    }   else if (!isNaN(round)) {
            selectedSet = module[`industry02060${round}`];
            roundTitleText = `흡배기 제어장치 ${round}회차`;
    }   else {
            return;
    }
...
function submitQuiz() {
    ...
    status.innerHTML = `<span id="scoreDisplay">흡배기제어장치 정비 ${round}회차 총점: ${score}/${questions.length}</span>`;

    document.getElementById("submitBtn").style.display = "none";

    const resultDiv = document.getElementById("result");
    resultDiv.innerHTML = `<h2>흡배기제어장치 정비 ${round}회차 총점: ${score}/${questions.length}</h2>`;
}
