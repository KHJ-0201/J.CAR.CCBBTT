...
async function loadQuestions() {
  try {
    const module = await import('./industry_안전문제.js');
    ...
    if (round === '0') {
        const allSets = [
            ...(module.industry070101 || []),
            ...(module.industry070102 || []),
            ...(module.industry070103 || []),
            ...(module.industry070104 || []),
            ...(module.industry070105 || []),
      ];
    selectedSet = shuffleArray(allSets).slice(0, 60);
    roundTitleText = '전기자동차 안전관리 전체 회차 랜덤 60문제';
    }   else if (!isNaN(round)) {
            selectedSet = module[`industry07010${round}`];
            roundTitleText = `전기자동차 안전관리 ${round}회차`;
    }   else {
            return;
    }
...
function submitQuiz() {
    ...
    status.innerHTML = `<span id="scoreDisplay">전기자동차 안전관리 ${round}회차 총점: ${score}/${questions.length}</span>`;

    document.getElementById("submitBtn").style.display = "none";

    const resultDiv = document.getElementById("result");
    resultDiv.innerHTML = `<h2>전기자동차 안전관리 ${round}회차 총점: ${score}/${questions.length}</h2>`;
}
