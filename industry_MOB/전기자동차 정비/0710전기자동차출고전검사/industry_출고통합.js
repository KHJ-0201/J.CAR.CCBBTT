...
async function loadQuestions() {
  try {
    const module = await import('./industry_출고문제.js');
    ...
    if (round === '0') {
        const allSets = [
            ...(module.industry071001 || []),
            ...(module.industry071002 || []),
            ...(module.industry071003 || []),
            ...(module.industry071004 || []),
            ...(module.industry071005 || []),
      ];
    selectedSet = shuffleArray(allSets).slice(0, 60);
    roundTitleText = '전기자동차 출고 전 검사 정비 전체 회차 랜덤 60문제';
    }   else if (!isNaN(round)) {
            selectedSet = module[`industry07100${round}`];
            roundTitleText = `전기자동차 출고 전 검사 정비 ${round}회차`;
    }   else {
            return;
    }
...
function submitQuiz() {
    ...
    status.innerHTML = `<span id="scoreDisplay">전기자동차 출고 전 검사 정비 ${round}회차 총점: ${score}/${questions.length}</span>`;

    document.getElementById("submitBtn").style.display = "none";

    const resultDiv = document.getElementById("result");
    resultDiv.innerHTML = `<h2>전기자동차 출고 전 검사 정비 ${round}회차 총점: ${score}/${questions.length}</h2>`;
}
