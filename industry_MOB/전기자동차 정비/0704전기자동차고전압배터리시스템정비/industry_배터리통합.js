...
async function loadQuestions() {
  try {
    const module = await import('./industry_배터리문제.js');
    ...
    if (round === '0') {
        const allSets = [
            ...(module.industry070401 || []),
            ...(module.industry070402 || []),
            ...(module.industry070403 || []),
            ...(module.industry070404 || []),
            ...(module.industry070405 || []),
      ];
    selectedSet = shuffleArray(allSets).slice(0, 60);
    roundTitleText = '전기자동차 고전압 배터리시스템 점검 전체 회차 랜덤 60문제';
    }   else if (!isNaN(round)) {
            selectedSet = module[`industry07040${round}`];
            roundTitleText = `전기자동차 고전압 배터리시스템 점검 ${round}회차`;
    }   else {
            return;
    }
...
function submitQuiz() {
    ...
    status.innerHTML = `<span id="scoreDisplay">전기자동차 고전압배터리시스템 점검 ${round}회차 총점: ${score}/${questions.length}</span>`;

    document.getElementById("submitBtn").style.display = "none";

    const resultDiv = document.getElementById("result");
    resultDiv.innerHTML = `<h2>전기자동차 고전압배터리시스템 점검 ${round}회차 총점: ${score}/${questions.length}</h2>`;
}
