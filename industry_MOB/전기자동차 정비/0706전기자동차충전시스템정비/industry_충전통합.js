...
async function loadQuestions() {
  try {
    const module = await import('./industry_충전문제.js');
    ...
    if (round === '0') {
        const allSets = [
            ...(module.industry070601 || []),
            ...(module.industry070602 || []),
            ...(module.industry070603 || []),
            ...(module.industry070604 || []),
            ...(module.industry070605 || []),
      ];
    selectedSet = shuffleArray(allSets).slice(0, 60);
    roundTitleText = '전기자동차 충전시스템 정비 전체 회차 랜덤 60문제';
    }   else if (!isNaN(round)) {
            selectedSet = module[`industry07060${round}`];
            roundTitleText = `전기자동차 충전시스템 정비 ${round}회차`;
    }   else {
            return;
    }
...
function submitQuiz() {
    ...
    status.innerHTML = `<span id="scoreDisplay">전기자동차 충전시스템 정비 ${round}회차 총점: ${score}/${questions.length}</span>`;

    document.getElementById("submitBtn").style.display = "none";

    const resultDiv = document.getElementById("result");
    resultDiv.innerHTML = `<h2>전기자동차 충전시스템 정비 ${round}회차 총점: ${score}/${questions.length}</h2>`;
}
