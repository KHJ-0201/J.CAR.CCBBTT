...
async function loadQuestions() {
  try {
    const module = await import('./industry_취급문제.js');
    ...
    if (round === '0') {
        const allSets = [
            ...(module.industry070201 || []),
            ...(module.industry070202 || []),
            ...(module.industry070203 || []),
            ...(module.industry070204 || []),
            ...(module.industry070205 || []),
      ];
    selectedSet = shuffleArray(allSets).slice(0, 60);
    roundTitleText = '전기자동차 고전압 취급관리 전체 회차 랜덤 60문제';
    }   else if (!isNaN(round)) {
            selectedSet = module[`industry07020${round}`];
            roundTitleText = `전기자동차 고전압 취급관리 ${round}회차`;
    }   else {
            return;
    }
...
function submitQuiz() {
    ...
    status.innerHTML = `<span id="scoreDisplay">전기자동차 고전압 취급관리 ${round}회차 총점: ${score}/${questions.length}</span>`;

    document.getElementById("submitBtn").style.display = "none";

    const resultDiv = document.getElementById("result");
    resultDiv.innerHTML = `<h2>전기자동차 고전압 취급관리 ${round}회차 총점: ${score}/${questions.length}</h2>`;
}
