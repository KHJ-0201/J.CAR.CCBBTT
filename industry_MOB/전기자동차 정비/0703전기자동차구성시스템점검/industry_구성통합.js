...
async function loadQuestions() {
  try {
    const module = await import('./industry_구성문제.js');
    ...
    if (round === '0') {
        const allSets = [
            ...(module.industry070301 || []),
            ...(module.industry070302 || []),
            ...(module.industry070303 || []),
            ...(module.industry070304 || []),
            ...(module.industry070305 || []),
      ];
    selectedSet = shuffleArray(allSets).slice(0, 60);
    roundTitleText = '전기자동차 구성시스템 점검 전체 회차 랜덤 60문제';
    }   else if (!isNaN(round)) {
            selectedSet = module[`industry07030${round}`];
            roundTitleText = `전기자동차 구성시스템 점검 ${round}회차`;
    }   else {
            return;
    }
...
function submitQuiz() {
    ...
    status.innerHTML = `<span id="scoreDisplay">전기자동차 구성시스템 점검 ${round}회차 총점: ${score}/${questions.length}</span>`;

    document.getElementById("submitBtn").style.display = "none";

    const resultDiv = document.getElementById("result");
    resultDiv.innerHTML = `<h2>전기자동차 구성시스템 점검 ${round}회차 총점: ${score}/${questions.length}</h2>`;
}
