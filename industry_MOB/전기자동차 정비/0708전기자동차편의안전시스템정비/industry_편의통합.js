...
async function loadQuestions() {
  try {
    const module = await import('./industry_편의문제.js');
    ...
    if (round === '0') {
        const allSets = [
            ...(module.industry070801 || []),
            ...(module.industry070802 || []),
            ...(module.industry070803 || []),
            ...(module.industry070804 || []),
            ...(module.industry070805 || []),
      ];
    selectedSet = shuffleArray(allSets).slice(0, 60);
    roundTitleText = '전기자동차 편의안전시스템 정비 전체 회차 랜덤 60문제';
    }   else if (!isNaN(round)) {
            selectedSet = module[`industry07080${round}`];
            roundTitleText = `전기자동차 편의안전시스템 정비 ${round}회차`;
    }   else {
            return;
    }
...
function submitQuiz() {
    ...
    status.innerHTML = `<span id="scoreDisplay">전기자동차 편의안전시스템 정비 ${round}회차 총점: ${score}/${questions.length}</span>`;

    document.getElementById("submitBtn").style.display = "none";

    const resultDiv = document.getElementById("result");
    resultDiv.innerHTML = `<h2>전기자동차 편의안전시스템 정비 ${round}회차 총점: ${score}/${questions.length}</h2>`;
}
