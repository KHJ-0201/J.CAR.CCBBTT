...
function startExamProcess(sets, isBalanced) {
    let pool = [];
    sets.forEach(set => {
        set.repairData.forEach(q => {
            q.originalRound = set.roundName;
            pool.push(q);
        });
    });
    ...
    if (isBalanced && questions.length > 80) {
...
function renderQuestion() {
    ...
    if (isOneByOne) {
        display.innerHTML = generateQuestionHTML(questions[currentIdx], currentIdx, isInstant);
    } else {
        display.innerHTML = questions.map((q, i) => generateQuestionHTML(q, i, isInstant)).join('');
    }
...
function selectAnswer(qIdx, aIdx) {
    ...
    if (document.getElementById('setting-auto-scroll').checked && !isInstant) {
        setTimeout(() => {
            if (document.getElementById('setting-one-by-one').checked) {
                moveQuestion(1);
            } else {
...
function updateProgressDisplay() {
    ...
    if (solvedEl && totalEl) {
        ...
        displayEl.textContent = solved + " / " + total;
    }
}
...
function handleFinishSubmit() {
    ...
    if (unsolvedCount > 0) {
        if (confirm(`미풀이 문제가 ${unsolvedCount}개 있습니다. 제출하시겠습니까?`)) {
            finalizeExam();
        } else {
            const firstUnsolved = questions.findIndex((_, i) => userAnswers[i] === undefined);
            if (firstUnsolved !== -1) {
                jumpTo(firstUnsolved);
            }
        }
    } else {
        finalizeExam();
...
function finalizeExam() {
    ...
    questions.forEach((q, i) => {
        ...
        card.innerHTML = `
            <h4>${isCorrect ? '✅' : '❌'} Q${i + 1}. ${q.question}</h4>
...
function handleOverlayClick(e) {
    if (e.target.id === 'modal-stats-overlay') {
        closeStatsModal();
    }
}
...
function retryCurrentExam(wrongOnly) {
    if (wrongOnly) {
        const wrongs = lastUsedExamData.filter((q, i) => {
            const userPick = questions[i].options[userAnswers[i]];
            return userPick !== questions[i].correctAnswerText;
        });
        if (wrongs.length === 0) return alert("틀린 문제가 없습니다!");
        questions = shuffle(wrongs);
    } else {
        questions = shuffle(lastUsedExamData);
    }
    document.getElementById('result-screen').classList.add('hidden');
    launchQuiz();
}
...
function renderOMR() {
    ...
    grid.innerHTML = questions.map((_, i) => `
        <div class="omr-row-item" id="omr-row-${i}" onclick="jumpTo(${i})">
            <span>${i + 1}</span>
            <div class="omr-circles">${[0,1,2,3].map(v => `<span class="circle-dot" id="dot-${i}-${v}"></span>`).join('')}</div>
        </div>
    `).join('');
}
...
function updateOMRMark(qIdx, aIdx) {
    ...
    if (row) {
        row.querySelectorAll('.circle-dot').forEach((dot, i) => {
            dot.classList.toggle('filled', i === aIdx);
        });
    }
    const el = document.getElementById('omr-status-txt');
    if (el) {
        el.innerText = `${Object.keys(userAnswers).length} / ${questions.length}`;
    }
}
...
function applyLeftHand(isOn) {
    document.getElementById('quiz-nav-group').classList.toggle('left-hand-mode-active', isOn);
}

function toggleViewMode() {
    if(questions.length > 0) launchQuiz();
}
...
function toggleWrongAccordion() {
    ...
    document.getElementById('wrong-arrow-icon').innerText = body.classList.contains('hidden') ? '▼' : '▲';
}
...
function saveWrongNotes(wrongs) {
    ...
    wrongs.forEach(q => {
        ...
        if (existingIdx !== -1) {
            data[round][existingIdx].wrongCount = (data[round][existingIdx].wrongCount || 1) + 1;
        } else {
            q.wrongCount = 1;
            data[round].push(q);
        }
...
function renderWrongNoteArea() {
    ...
    container.innerHTML = rounds.map(r => `
        <div class="wrong-item-row" style="margin-bottom:10px; display:flex; align-items:center; justify-content:space-between;">
            <span>${r} (${data[r].length}개)</span>
            <div>
                <button onclick="startWrongReview('${r}')" style="background:var(--accent-blue); color:white; padding:5px 10px; border-radius:6px;">풀기</button>
                <button class="btn-round-del" onclick="deleteWrongRound('${r}')">삭제</button>
            </div>
        </div>
    `).join('');
}
...
function startWrongReview(roundName) {
    const data = JSON.parse(localStorage.getItem('cbt_wrong_v4') || '{}');
    // [수정] 오답 노트를 풀기 시작할 때 재시험용 데이터 탱크(lastUsedExamData)에 저장
...
function openStatsModal() {
    ...
    if (history.length === 0) {
        container.innerHTML = '<p style="padding:50px;">데이터가 없습니다.</p>';
        return;
    }

    container.innerHTML = history.map(h => `
        <div class="chart-bar" style="height: ${h.score}%">
            <span class="bar-score">${h.score.toFixed(2)}점</span>
            <span class="bar-label">${h.roundName}<br><small style="font-size:0.6rem; color:#888;">${h.date || ''}</small></span>
        </div>
    `).join('');
...
function closeStatsModal() {
    document.getElementById('modal-stats-overlay').classList.add('hidden');
}

function handleSecretReset() {
    if (++secretResetCount >= 5) {
        if (confirm("초기화하시겠습니까?")) {
            localStorage.removeItem('cbt_history_v4');
            location.reload();
        }
        secretResetCount = 0;
    }
