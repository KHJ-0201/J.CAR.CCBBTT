...
export const industry030301 = [
    {
        question: "냉매 순환 시스템에서 액체 냉매 내에 포함된 수분을 흡수하여 시스템의 부식을 방지하고, 이물질을 걸러주는 역할을 하는 부품은 무엇입습니까?",
...
function renderQuiz() {
    ...
    questions.forEach((q, i) => {
      ...
      // 1. imagePath 속성이 있는지 확인 (사용자 코드 유지)
        if (q.imagePath) {
            div.innerHTML += `<img 
                src="${q.imagePath}" 
                alt="문제 그림" 
...
function submitQuiz() {
    ...
    questions.forEach((q, i) => {
      ...
      if (answers[i] == q.answer) {
          // 정답: 초록색 테두리 동그라미 (⭕ 사용, CSS로 초록색 입힘)
          score++;
          labels[q.answer].style.backgroundColor = "#b6fcb6";
          statusSpan.innerHTML = '⭕'; 
          // statusSpan.style.color는 CSS에서 정의
          statusSpan.classList.add('status-correct'); // 클래스 추가
      } else if (answers[i] >= 0) {
          // 오답: ❌ (빨간색)
          labels[q.answer].style.backgroundColor = "#b6fcb6"; 
          ...
          statusSpan.classList.add('status-wrong'); // 클래스 추가
      } else {
          // 미응답: ❓ (노란색)
          labels[q.answer].style.backgroundColor = "#b6fcb6"; 
          statusSpan.innerHTML = '❓'; 
          statusSpan.style.color = '#ffc107'; 
          statusSpan.classList.add('status-unanswered'); // 미세 조정을 위한 클래스 추가
      }
