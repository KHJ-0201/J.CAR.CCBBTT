...
async function loadQuestions() {
  try {
    const module = await import('./industry_냉난방문제.js');

    let selectedSet;

    if (round === '0') {
        const allSets = [
            ...(module.industry010301 || []),
            ...(module.industry010302 || []),
            ...(module.industry010303 || []),
            ...(module.industry010304 || []),
...
function submitQuiz() {
    ...
    status.innerHTML = `<span id="scoreDisplay">냉난방장치정비 ${round}회차 총점: ${score}/${questions.length}</span>`;

    document.getElementById("submitBtn").style.display = "none";

    const resultDiv = document.getElementById("result");
    resultDiv.innerHTML = `<h2>냉난방장치정비 ${round}회차 총점: ${score}/${questions.length}</h2>`;
}
