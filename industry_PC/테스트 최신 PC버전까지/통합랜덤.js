...
let lastWrongAnswers = []; // [수정] 무한 오답 풀기를 위해 방금 틀린 문제만 따로 보관
    ...
    if (isBalanced && sets.length > 0) {
        ...
        sets.forEach(set => {
            set.repairData.forEach(q => {
                pool.push({ ...q, originalRound: set.roundName });
            });
        });
    }
    
    // [수정] 최초 시작 시 문제지 원본 저장
...
function finalizeExam() {
    ...
    questions.forEach((q, i) => {
        ...
        if (isCorrect) {
            ...
            // [수정] 오답의 오답 풀기를 위해 문제 원본(보기 셔플 전)을 저장
            currentWrongs.push({
                ...q,
                options: q.correctAnswerText ? q.options : q.options // 원본 텍스트 유지를 위해 객체 복사
            });
        }
        const card = document.createElement('div');
    ...
    // [수정] 현재 시험지의 전체 구성 보존 (전체 다시 풀기용)
    lastUsedExamData = JSON.parse(JSON.stringify(questions.map(q => ({...q, options: q.options})))); 
    // [수정] 방금 틀린 문제들만 따로 보관 (오답만 다시 풀기용)
...
function retryCurrentExam(wrongOnly) {
    ...
    if (wrongOnly) {
        ...
        // [수정] 직전에 풀었던 시험지 구성 전체 사용
        targetList = JSON.parse(JSON.stringify(lastUsedExamData));
    }

    // [수정] 문제와 보기 순서를 완전히 새롭게 셔플하여 다시 시작
...
function pushAllAnswers(answerIdx) {
    ...
    questions.forEach((_, i) => {
        if (userAnswers[i] === undefined) { selectAnswer(i, answerIdx); }
    });
}
