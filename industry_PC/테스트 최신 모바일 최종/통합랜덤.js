...
function renderRoundCards() {
    ...
    // [수정] 문제 데이터(repairData)가 있는 회차만 필터링하여 노출합니다.
...
function startExam(type) {
    let sets = [];
    if (type === 'full') {
        // [수정] 전체 랜덤 시에도 데이터가 있는 회차만 대상으로 합니다.
...
function startExamProcess(sets, isBalanced) {
    ...
    if (isBalanced && sets.length > 0) {
        // [수정] 각 회차별로 균일하게 문제를 추출하여 최대 80문항을 맞춥니다.
        ...
        sets.forEach(set => {
            ...
            pool.push(...shuffledSet.slice(0, count).map(q => ({
                ...q,
                originalRound: set.roundName
            })));
        });
    } else {
        sets.forEach(set => {
...
function renderWrongNoteArea() {
    ...
    const rounds = Object.keys(data).sort((a, b) => {
    ...
    container.innerHTML = rounds.map(r => `
        <div class="wrong-item-row" id="wrong-row-${r}">
            <div style="display:flex; align-items:center; justify-content:space-between;">
                ...
                <div>
                    <button onclick="startWrongReview('${r}')" style="background:var(--accent-blue); color:white; padding:4px 8px; border-radius:6px; font-size:0.75rem;">단독 풀기</button>
...
function updateWrongStartBtn() {
    ...
    if (btn) {
        if (selectedCount > 0) {
            ...
            btn.innerText = `🚀 선택 ${selectedCount}개 모아풀기`;
        } else {
...
function startMultiWrongReview() {
    ...
    let pool = [];
    // [수정] 오답 모아 풀기 시에도 각 회차별로 균일하게 추출하여 최대 80문항을 맞춥니다.
    const targetTotal = 80;
